...
function custom_pxecfg
{
  echo "custom_pxecfg..."
	...
	#将default下的ks配置删除
	sed -i "s/ks=.*pxe_kickstart.cfg//g" $DEFAULT_CFG
...
function start_pxesvr
{
	...
	if [ `service dhcpd status | grep -c running` == 0 ]; then
		...
		echo "Inatall Environment Prepare Success! Have fun!"
	else
		service dhcpd restart
		echo "Warning............................"
		echo "Warning............................"
		echo "Warning: other DHCP is Running. Restart DHCP....."
		echo "Please Check Current Dhcp config. Sure Current Dhcp Can Auto Install ISO"
		echo "Warning............................"
		echo "Warning............................"
	fi
